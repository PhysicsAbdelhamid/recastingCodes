...
if mass_Phi >= 400: # Condition if the sample is "High-ET" or " Low-ET"
    print('eff eval with Map')
    #map_eff_highETX = cmfp.eff_map_High(pT_DH1, eta_DH1, Lxy_tot_DH1, Lz_tot_DH1, abs(pdg_tot_DH1), pT_DH2, eta_DH2, Lxy_tot_DH2, Lz_tot_DH2, abs(pdg_tot_DH2), tauN, Nevent,  mass_Phi, mass_S) # Compute the efficiency from Pythia
    map_eff_highETX = np.zeros(len(tauN))
    print('Map eval with BDT')
    eff_highETX = cmfp.eff_bdt_High(pT_DH1, eta_DH1, Lxy_tot_DH1, Lz_tot_DH1, abs(pdg_tot_DH1), pT_DH2, eta_DH2, Lxy_tot_DH2, Lz_tot_DH2, abs(pdg_tot_DH2),  E_DH1, E_DH2,tauN, Nevent,  mass_Phi, mass_S) # Compute the efficiency from Pythia
    print('Plotting...')
    cmfp.plt_eff(map_eff_highETX, eff_highETX, tauN, data_HEP, mass_Phi, mass_S ) # Ploting and saving a comparison of all the results of efficiencies
    ...
    #map_eff_lowETX = cmfp.eff_map_Low(pT_DH1, eta_DH1, Lxy_tot_DH1, Lz_tot_DH1, abs(pdg_tot_DH1), pT_DH2, eta_DH2, Lxy_tot_DH2, Lz_tot_DH2, abs(pdg_tot_DH2), tauN, Nevent, mass_Phi, mass_S)
    map_eff_lowETX = np.zeros(len(tauN))
    eff_lowETX = cmfp.eff_bdt_Low(pT_DH1, eta_DH1, Lxy_tot_DH1, Lz_tot_DH1, abs(pdg_tot_DH1), pT_DH2, eta_DH2, Lxy_tot_DH2, Lz_tot_DH2, abs(pdg_tot_DH2),  E_DH1,  E_DH2, tauN, Nevent, mass_Phi, mass_S)
    cmfp.plt_eff(map_eff_lowETX, eff_lowETX, tauN, data_HEP, mass_Phi, mass_S)
    cmfp.plt_cross(eff_lowETX, tauN, mass_Phi, mass_S, branch_HEP_limit, factor, hepdata_eff=data_HEP)
