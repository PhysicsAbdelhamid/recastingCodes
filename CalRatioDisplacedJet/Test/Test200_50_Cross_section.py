...
fichier_root = uproot.open("ATLAS_data/HEP_Limits/HEPData-ins2043503-v3-Figure_5f_of_Aux._Mat..root")
objet_graphe = fichier_root["Figure 5f of Aux. Mat./Graph1D_y2;1"]
#objet_graphe = fichier_root["Figure 2c of Aux. Mat. 200_50/Graph1D_y1;1"]


effMG_pythia = "Plots_Low/HAHM_old_version/200_50/Cross_section/Cross_section_mH200_mS50_nevents10000.txt"
effMG_pythia_data = np.loadtxt(effMG_pythia)

ctau = "Plots_Low/HAHM_old_version/200_50/Cross_section/Cross_section_mH200_mS50_nevents10000_ctau.txt"
...
ax.text(0.19, 0.83, f" $ m_Φ $ = 200 GeV, $m_S$ = 50 GeV" , transform=ax.transAxes, fontsize=10, verticalalignment='top', bbox=props)
ax.text(0.19, 0.74, f"$Nevents$ = 10000", transform=ax.transAxes, fontsize=10, verticalalignment='top', bbox=props)
x = np.linspace(0,100)
#ax.fill_between(x, 0.25*(max(effMG_pythia_data)), color='black', alpha=.2, hatch="/", edgecolor="black", linewidth=1.0) # adding hatch
plt.ylim(10e-3,10e1) # start at 0
plt.xlim(1e-1, 79.517)
plt.xscale('log')
plt.yscale('log')
plt.xlabel(r'c$\tau$ [m]', fontsize=13)
plt.ylabel(r'95% CL limit on $\sigma \times B$ [pb]', fontsize=13 )
plt.legend( fontsize = 9, loc='upper center', bbox_to_anchor=(0.5, 1)) # set the legend in the upper right corner
